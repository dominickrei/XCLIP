...
class Hyperformer_Model(nn.Module):
    def __init__(self, graph_class, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        warnings.warn('joint_label is hardcoded in this model to save time. Thus, it can only be applied to the NTU dataset. Make this a cfg option later')
        ...
        self.clip_projector = nn.Linear(24*num_of_heads, 512)
        ...
        nn.init.normal_(self.clip_projector.weight, 0, math.sqrt(2. / 512))
        bn_init(self.data_bn, 1)
        if drop_out:
    ...
    def forward(self, x): #y
        ...
        ### Initial code for pooling skeletons
        # # spatial temporal average pooling
        # x = x.view(N, M, C, -1)
        # x = x.mean(3).mean(1)
        # # x = self.drop_out(x)
        # # x1 = self.fc(x)
        # x2 = self.clip_projector(x)

        ### Dominick's code for pooling skeletons
        ### Output the features for each joint so we can use them in the 'Video specific prompting' module
        x = x.view(N, M, C, T, V)
        x = x.mean(3).mean(1)
        x = x.view(N, V, C)
        x2 = self.clip_projector(x) # shape is (N, V, 512)
