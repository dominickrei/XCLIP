...
def main(config): 
    ...
    if config.TEST.ONLY_TEST:
        acc1, mca = validate(val_loader, text_labels, model, config)
...
def validate(val_loader, text_labels, model, config):
    ...
    mca_meter = MetricMeterMCA(config.DATA.NUM_CLASSES)
    ...
    logger.info(f' * Acc@1 {acc1_meter.avg:.3f} Acc@5 {acc5_meter.avg:.3f} mCA {mca_meter.mca:.3f}')
    return acc1_meter.avg, mca_meter.mca
